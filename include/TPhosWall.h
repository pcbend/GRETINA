...
class TPhosWall : public TObject {
  ...
  public:
    ...
    const inline Int_t    GetBDiff()    {if(Size()<1) return -1; return BDiffCal(GetLargestNumber());}
    ...
    const inline Int_t    GetPixel()    { if(Size()<1) return -1; return Pixel(GetLargestNumber()); }
