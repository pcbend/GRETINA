//g++ main.cxx -I${GEBSYS}/include -L${GEBSYS}/libraries -lG2Format -lPWFormat -lGEBFormat `root-config --cflags --libs` 
...
int main(int argc, char **argv) {
  ...
  while(run) {
    ...
    if(ptime>gtime) {
      ...
      pbytes = ReadParticle(pfile,event);
      if(!pbytes) { 
        pwf_counter++;
      ...
      ptotal += pbytes; 
      ptime = event.GetTimeStamp();
      current = ptime;
    }
    //printf( "abs(current-start)  =  %i  \n", abs(current-start));
    if(abs(current-start)>200) {   
      start = current;
      tree->Fill();
      gretina->Clear();        
      phoswall->Clear();
      ...
      switch(event.GetEventType()) {
        case 1:
          gretina->AddGretinaHit(G2Fragment(event));
          break;
        case 17:
          phoswall->AddPWHit(PWFragment(event));
          break;
      };
    }
